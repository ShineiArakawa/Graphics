...
GLuint vaoId_background;
GLuint vertexBufferId_background;
GLuint indexBufferId_background;
...
void initVAOBackground() {
	...
	const float indexes[2][3] = {
		{0, 1, 2},
		{1, 2, 3}
	};
	glm::vec3 colorsBackGround[4] = {
	...
	for (int j = 0; j < 3; j++) {
		Vertex v(positions[j], colorsBackGround[j]);
		vertices.push_back(v);
		indices.push_back(idx++);
	}

	for (int j = 0; j < 3; j++) {
		Vertex v(positions[j + 1], colorsBackGround[j + 1]);
		vertices.push_back(v);
		indices.push_back(idx++);
	}
	...
	// 頂点バッファオブジェクトの作成
	// Create vertex buffer object
	glGenBuffers(1, &vertexBufferId_background);
	glBindBuffer(GL_ARRAY_BUFFER, vertexBufferId_background);
	glBufferData(GL_ARRAY_BUFFER, sizeof(Vertex) * vertices.size(), vertices.data(), GL_STATIC_DRAW);
	...
	// 頂点番号バッファオブジェクトの作成
	// Create index buffer object
	glGenBuffers(1, &indexBufferId_background);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, indexBufferId_background);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned int) * indices.size(),
		indices.data(), GL_STATIC_DRAW);
