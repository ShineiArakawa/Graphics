...
static const float DISTANCE_OF_CUBES = 5;
...
std::vector<TransformVariables> transformVariables;
...
void initVAO() {
    for (int i = 0; i < NUM_CUBES; i++) {
        GLuint vaoId = i;
        GLuint vertexBufferId = i;
        GLuint indexBufferId = i;

        glm::vec3 offset(0);
        offset.x = DISTANCE_OF_CUBES * (float)i;
        ...
        for (int i = 0; i < 6; i++) {
            for (int j = 0; j < 3; j++) {
                Vertex v(positions[faces[i * 2 + 0][j]] + offset, colors[i]);
                vertices.push_back(v);
                indices.push_back(idx++);
            }

            for (int j = 0; j < 3; j++) {
                Vertex v(positions[faces[i * 2 + 1][j]] + offset, colors[i]);
                vertices.push_back(v);
                indices.push_back(idx++);
            }
        }
...
void initTransformVariables() {
    transformVariables.clear();

    for (int i = 0; i < NUM_CUBES; i++) {
        TransformVariables tranVal(&WIN_WIDTH, &WIN_HEIGHT);
        transformVariables.push_back(tranVal);
    }
}
